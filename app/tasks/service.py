...
class TaskService:
    ...
    async def get_tasks(self) -> list[TaskSchema]:
        cache_tasks = await self.task_cache.get_tasks()
        if cache_tasks:
            return cache_tasks
    ...
    async def update_task_name(self, task_id: int, name: str, user_id: int) -> TaskSchema:
        task = await self.task_repository.get_user_tasks(task_id, user_id)
        if not task:
            raise TaskNotFound
        updated_task = await self.task_repository.update_task_name(task_id, name)
